...
// clusters is a literal address the API server cert may not carry as an IP SAN.
// apiServerUrlViaServiceDns() decides whether to swap in the kubernetes.default.svc
// DNS name (always in the cert SANs) instead.
describe('apiServerUrlViaServiceDns — IPv6 API server TLS SAN workaround', () => {
  ...
  it('auto: rewrites to service DNS when the host is an IPv6 literal', () => {
    assert.equal(apiServerUrlViaServiceDns({host: 'fd00::1', port: '443'}), 'https://kubernetes.default.svc:443')
  })

  it('auto: leaves IPv4 clusters untouched', () => {
    assert.equal(apiServerUrlViaServiceDns({host: '10.96.0.1', port: '443'}), null)
  })
  ...
  it('preserves the port and picks http for the plain-text API ports', () => {
    assert.equal(apiServerUrlViaServiceDns({host: 'fd00::1', port: '8080'}), 'http://kubernetes.default.svc:8080')
  })

  it('returns null when not running in a cluster (no host)', () => {
    assert.equal(apiServerUrlViaServiceDns({host: undefined, port: '443'}), null)
  })

  it('reads host/port/mode from the environment by default', () => {
    process.env.KUBERNETES_SERVICE_HOST = 'fd00::1'
    process.env.KUBERNETES_SERVICE_PORT = '443'
    assert.equal(apiServerUrlViaServiceDns(), 'https://kubernetes.default.svc:443')
  })
})
